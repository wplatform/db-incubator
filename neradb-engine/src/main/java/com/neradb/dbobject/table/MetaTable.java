...
import com.neradb.dbobject.constraint.ConstraintCheck;
import com.neradb.dbobject.constraint.ConstraintReferential;
import com.neradb.dbobject.constraint.ConstraintUnique;
import com.neradb.dbobject.index.Index;
import com.neradb.dbobject.index.IndexType;
import com.neradb.dbobject.index.MetaIndex;
...
import com.neradb.store.InDoubtTransaction;
import com.neradb.tools.Csv;
...
public class MetaTable extends Table {
	...
	public MetaTable(Schema schema, int id, int type) {
		...
		case SEQUENCES:
			...
			cols = createColumns("SEQUENCE_CATALOG", "SEQUENCE_SCHEMA", "SEQUENCE_NAME", "CURRENT_VALUE BIGINT",
					"INCREMENT BIGINT", "IS_GENERATED BIT", "REMARKS", "CACHE BIGINT", "MIN_VALUE BIGINT",
					"MAX_VALUE BIGINT", "IS_CYCLE BIT", "ID INT");
			break;
		case USERS:
			setObjectName("USERS");
	...
	/**
	 * Generate the data for the given metadata table using the given first and last
	 * row filters.
	 *
	 * @param session
	 *            the session
	...
	/**
	 * Get the number of meta table types. Supported meta table types are 0 .. this
	 * value - 1.
	 *
