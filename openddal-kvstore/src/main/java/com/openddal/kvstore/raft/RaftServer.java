...
public class RaftServer implements RaftMessageHandler {
	...
	private final static Logger logger = LoggerFactory.getLogger(RaftClient.class);
    ...
    public RaftServer(RaftContext context){
        ...
        for(long i = Math.max(this.state.getCommitIndex() + 1, this.logStore.getStartIndex()); i < this.logStore.getFirstAvailableIndex(); ++i){
            ...
            if(logEntry.getValueType() == LogValueType.Configuration){
                logger.info("detect a configuration change that is not committed yet at index %d", i);
        ...
        logger.info("Server %d started", this.id);
    }
    ...
    private synchronized RaftResponseMessage handleAppendEntriesRequest(RaftRequestMessage request){
        ...
        if(request.getTerm() == this.state.getTerm()){
            if(this.role == ServerRole.Candidate){
                ...
                logger.error("Receive AppendEntriesRequest from another leader(%d) with same term, there must be a bug, server exits", request.getSource());
        ...
        if(request.getLogEntries() != null && request.getLogEntries().length > 0){
            ...
            while(index < this.logStore.getFirstAvailableIndex() && logIndex < logEntries.length){
                ...
                if(oldEntry.getValueType() == LogValueType.Application){
                    ...
                    logger.info("revert a previous config change to config at %d", this.config.getLogIndex());
                ...
                if (logEntry.getValueType() == LogValueType.Application) {
                    ...
                    logger.info("received a configuration change at index %d from leader", index);
                    this.configChanging = true;
                }
            ...
            while(logIndex < logEntries.length){
                ...
                if(logEntry.getValueType() == LogValueType.Configuration){
                    logger.info("received a configuration change at index %d from leader", indexForEntry);
                    this.configChanging = true;
    ...
    private synchronized void handleElectionTimeout(){
        if(this.steppingDown > 0){
            ...
            logger.info("stepping down (cycles left: %d), skip this election timeout event", this.steppingDown);
            this.restartElectionTimer();
            return;
        }
    ...
    private void requestVote(){
        // vote for self
        logger.info("requestVote started with term %d", this.state.getTerm());
        ...
        for(PeerServer peer : this.peers.values()){
            ...
            logger.debug("send %s to server %d with term %d", RaftMessageType.RequestVoteRequest.toString(), peer.getId(), this.state.getTerm());
            ...
            peer.SendRequest(this.createAppendEntriesRequest(peer))
                .whenCompleteAsync((RaftResponseMessage response, Throwable error) -> {
                    try{
                        ...
                        logger.error("Uncaught exception %s", err.toString());
                    }
                }, this.context.getScheduledExecutor());
            return true;
        }

        logger.debug("Server %d is busy, skip the request", peer.getId());
        return false;
    }

    private synchronized void handlePeerResponse(RaftResponseMessage response, Throwable error){
        if(error != null){
            logger.info("peer response error: %s", error.getMessage());
            return;
        }
        ...
        if(response.getTerm() < this.state.getTerm()){
            logger.info("Received a peer response from %d that with lower term value %d v.s. %d", response.getSource(), response.getTerm(), this.state.getTerm());
            return;
        }
        ...
        if(response.getMessageType() == RaftMessageType.RequestVoteResponse){
            ...
            logger.error("Received an unexpected message %s for response, system exits.", response.getMessageType().toString());
            this.stateMachine.exit(-1);
        }
    }
    ...
    private void handleAppendEntriesResponse(RaftResponseMessage response){
        ...
        if(peer == null){
            logger.info("the response is from an unkonw peer %d", response.getSource());
            return;
        }
            ...
            synchronized(peer){
                ...
                if(context == null){
                    ...
                    if(response.getNextIndex() >= context.getSnapshot().getSize()){
                        ...
                        logger.debug("continue to sync snapshot at offset %d", response.getNextIndex());
    ...
    private void handleVotingResponse(RaftResponseMessage response){
        ...
        if(this.votesGranted > (this.peers.size() + 1) / 2){
            logger.info("Server is elected as leader for term %d", this.state.getTerm());
            this.electionCompleted = true;
            this.becomeLeader();
        }
    }
    ...
    private synchronized void handleHeartbeatTimeout(PeerServer peer){
        logger.debug("Heartbeat timeout for %d", peer.getId());
        if(this.role == ServerRole.Leader){
            this.requestAppendEntries(peer);
            ...
            synchronized(peer){
                if(peer.isHeartbeatEnabled()){
                    ...
                    logger.debug("heartbeat is disabled for peer %d", peer.getId());
                }
            }
        }else{
            logger.info("Receive a heartbeat event for %d while no longer as a leader", peer.getId());
        }
    }
        ...
        try{
            ...
            if(this.context.getRaftParameters().getSnapshotDistance() > 0
                ...
                if(currentSnapshot != null && indexCommitted - currentSnapshot.getLastLogIndex() < this.context.getRaftParameters().getSnapshotDistance()){
                    logger.info("a very recent snapshot is available at index %d, will skip this one", currentSnapshot.getLastLogIndex());
                    this.snapshotInProgress.set(0);
                    snapshotInAction = false;
                }else{
                    logger.info("creating a snapshot for index %d", indexCommitted);
                    ...
                    this.stateMachine.createSnapshot(snapshot).whenCompleteAsync((Boolean result, Throwable error) -> {
                        try{
                            if(error != null){
                                logger.error("failed to create a snapshot due to %s", error.getMessage());
                                return;
                            }
            ...
            logger.error("failed to compact logs at index %d, due to errors %s", indexCommitted, error.toString());
            if(snapshotInAction){
    ...
    private RaftRequestMessage createAppendEntriesRequest(PeerServer peer){
        ...
        if(lastLogIndex >= currentNextIndex){
            logger.error("Peer's lastLogIndex is too large %d v.s. %d, server exits", lastLogIndex, currentNextIndex);
            this.stateMachine.exit(-1);
        }
    ...
    private void reconfigure(ClusterConfiguration newConfig){
        ...
        for(ClusterServer server : serversAdded){
            if(server.getId() != this.id){
                ...
                logger.info("server %d is added to cluster", peer.getId());
                if(this.role == ServerRole.Leader){
                    logger.info("enable heartbeating for server %d", peer.getId());
        ...
        for(Integer id : serversRemoved){
            ...
            if(peer == null){
                logger.info("peer %d cannot be found in current peer list", id);
            } else{
                ...
                logger.info("server %d is removed from cluster", id.intValue());
            }
        }
    ...
    private synchronized RaftResponseMessage handleExtendedMessages(RaftRequestMessage request){
        if(request.getMessageType() == RaftMessageType.AddServerRequest){
            ...
            logger.error("receive an unknown request %s, for safety, step down.", request.getMessageType().toString());
            this.stateMachine.exit(-1);
        }
    ...
    private RaftResponseMessage handleInstallSnapshotRequest(RaftRequestMessage request){
        ...
        if(request.getTerm() == this.state.getTerm() && !this.catchingUp){
            if(this.role == ServerRole.Candidate){
                ...
                logger.error("Receive InstallSnapshotRequest from another leader(%d) with same term, there must be a bug, server exits", request.getSource());
                this.stateMachine.exit(-1);
            }else{
                this.restartElectionTimer();
            }
    ...
    private boolean handleSnapshotSyncRequest(SnapshotSyncRequest snapshotSyncRequest){
        try{
            ...
            logger.error("I/O error %s while saving the snapshot or applying the snapshot, no reason to continue", error.getMessage());
            this.stateMachine.exit(-1);
            return false;
        }
    ...
    private synchronized void handleExtendedResponse(RaftResponseMessage response, Throwable error){
        ...
        if(response.getMessageType() == RaftMessageType.SyncLogResponse){
            ...
            if(response.getNextIndex() >= context.getSnapshot().getSize()){
                ...
                context.setOffset(response.getNextIndex());
                logger.debug("continue to send snapshot to new server at offset %d", response.getNextIndex());
            }

            this.syncLogsToNewComingServer(this.serverToJoin.getNextLogIndex());
        }else{
            // No more response message types need to be handled
            logger.error("received an unexpected response message type %s, for safety, stepping down", response.getMessageType());
            this.stateMachine.exit(-1);
        }
    }

    private void handleExtendedResponseError(Throwable error){
        logger.info("receive an error response from peer server, %s", error.toString());
                ...
                if(server != null){
                    if(server.getCurrentHeartbeatInterval() >= this.context.getRaftParameters().getMaxHeartbeatInterval()){
                        if(request.getMessageType() == RaftMessageType.LeaveClusterRequest){
                            logger.info("rpc failed again for the removing server (%d), will remove this server directly", server.getId());
                            ...
                            if(this.peers.size() == 1){
                                ...
                                if(peer == null){
                                    ...
                                    logger.info("server %d is removed from cluster", server.getId());
                                }
                            }
                            
                            this.removeServerFromCluster(server.getId());
                        }else{
                            logger.info("rpc failed again for the new coming server (%d), will stop retry for this server", server.getId());
                            this.configChanging = false;
                            this.serverToJoin = null;
                        }
                    }else{
    ...
    private RaftResponseMessage handleRemoveServerRequest(RaftRequestMessage request){
        ...
        if(peer == null){
            logger.info("server %d does not exist", serverId);
            return response;
        }
    ...
    private RaftResponseMessage handleAddServerRequest(RaftRequestMessage request){
        ...
        if(this.peers.containsKey(server.getId()) || this.id == server.getId()){
            logger.warn("the server to be added has a duplicated id with existing server %d", server.getId());
            return response;
        }
    ...
    private void syncLogsToNewComingServer(long startIndex){
        ...
        if(gap < this.context.getRaftParameters().getLogSyncStopGap()){
            ...
            logger.info("LogSync is done for server %d with log gap %d, now put the server into cluster", this.serverToJoin.getId(), gap);
    ...
    private void removeServerFromCluster(int serverId){
        ...
        logger.info("removed a server from configuration and save the configuration to log store at %d", newConfig.getLogIndex());
        this.configChanging = true;
        ...
        synchronized(peer){
            ...
            if(snapshot == null || (lastSnapshot != null && lastSnapshot.getLastLogIndex() > snapshot.getLastLogIndex())){
                ...
                if(snapshot == null || lastLogIndex > snapshot.getLastLogIndex()){
                    logger.error("system is running into fatal errors, failed to find a snapshot for peer %d(snapshot null: %s, snapshot doesn't contais lastLogIndex: %s)", peer.getId(), String.valueOf(snapshot == null), String.valueOf(lastLogIndex > snapshot.getLastLogIndex()));
                    this.stateMachine.exit(-1);
                    return null;
                }
                ...
                logger.info("trying to sync snapshot with last index %d to peer %d", snapshot.getLastLogIndex(), peer.getId());
            ...
            try{
                ...
                if(sizeRead < data.length){
                    logger.error("only %d bytes could be read from snapshot while %d bytes are expected, should be something wrong" , sizeRead, data.length);
                    this.stateMachine.exit(-1);
                    return null;
                }
            }catch(Throwable error){
                // if there is i/o error, no reason to continue
                logger.error("failed to read snapshot data due to io error %s", error.toString());
                this.stateMachine.exit(-1);
                return null;
            }
    ...
    static class RaftMessageSenderImpl implements RaftMessageSender {
        ...
        private CompletableFuture<Boolean> sendMessageToLeader(RaftRequestMessage request){
            ...
            rpcClient.send(request).whenCompleteAsync((RaftResponseMessage response, Throwable err) -> {
                if(err != null){
                    logger.info("Received an rpc error %s while sending a request to server (%d)", err.getMessage(), leaderId);
                    result.complete(false);
                }else{
        ...
        public void run() {
            while(true){
                try{
                    ...
                    while(currentCommitIndex < server.quickCommitIndex && currentCommitIndex < server.logStore.getFirstAvailableIndex() - 1){
                        ...
                        if(logEntry.getValueType() == LogValueType.Application){
                            ...
                            synchronized(server){
                                ...
                                logger.info("configuration at index %d is committed", newConfig.getLogIndex());
                    ...
                    logger.error("error %s encountered for committing thread, which should not happen, according to this, state machine may not have further progress, stop the system", error, error.getMessage());
